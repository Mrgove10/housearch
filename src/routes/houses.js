...
router.post('/houses', async (req, res) => {
  ...
  const info = db.prepare(
    `INSERT INTO house (title, source_url, source_site, address, lat, lng, price, surface_m2, rooms, bedrooms, year_built, dpe, lot_m2, description, raw_json, contact_name, contact_phone, contact_email)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
  ).run(
    ...
    b.year_built || null, b.dpe || null, b.lot_m2 || null, b.description || null, b.raw_json || null,
...
router.post('/houses/:id', async (req, res) => {
  ...
  db.prepare(
    `UPDATE house SET title=?, source_url=?, source_site=?, address=?, lat=?, lng=?, geo_precise=?, price=?, surface_m2=?, rooms=?, bedrooms=?, year_built=?, dpe=?, lot_m2=?, description=?, contact_name=?, contact_phone=?, contact_email=? WHERE id=?`
  ).run(
    ...
    b.year_built || null, b.dpe || null, b.lot_m2 || null, b.description || null,
